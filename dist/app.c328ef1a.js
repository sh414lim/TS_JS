...
var ajax = new XMLHttpRequest(); // 출력결과 반환
...
function newsFeed() {
  ...
  for (var i = (store.currentPage - 1) * 10; i < store.currentPage * 10; i++) {
    var title = newsFeed[i].title;
    var desc = newsFeed[i].comments_count;
    var id = newsFeed[i].id;
  ...
  templage = templage.replace("{{__next_page__}}", store.currentPage > 1 ? store.currentPage - 1 : 1);
  templage = templage.replace("{{__prev_page__}}", store.currentPage < 3 ? store.currentPage + 1 : 3);
  container.innerHTML = templage; // 하나의 문자열 조인
}

function newsDetail() {
  var id = location.hash.substr(7);
  var url = CONTENT_URL.replace("@id", id);
  var newsContent = getDate(url);
  var subTitle = newsContent.title;
